...
function template_latest_news()
{
	...
	echo '
		<div class="cat_bar">
			<h3 class="catbg">
				<span class="ie6_header floatleft"><a href="', $scripturl, '?action=helpadmin;help=live_news" onclick="return reqWin(this.href);" class="help"><i style="color: white;" class="fas fa-bullhorn"></i></a> ', $txt['mc_latest_news'], '</span>
			</h3>
		</div>
		<div class="windowbg">
			<div class="content">
...
function template_moderation_settings()
{
	...
	foreach ($context['homepage_blocks'] as $k => $v)
		echo '<input type="checkbox" id="mod_homepage_', $k, '" name="mod_homepage[', $k, ']"', in_array($k, $context['mod_settings']['user_blocks']) ? ' checked="checked"' : '', ' class="input_check" />
							<label for="mod_homepage_', $k, '">', $v, '</label>
	...
	if ($context['can_moderate_boards'])
	{
		echo '
						...
						<dd>
							<input type="checkbox" id="mod_show_reports" name="mod_show_reports" ', $context['mod_settings']['show_reports'] ? 'checked="checked"' : '', ' class="input_check" /><label for="mod_show_reports"></label>
						</dd>
						<dt>
