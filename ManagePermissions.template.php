...
function template_permission_index()
{
		...
		echo '
			...
			echo '
						...
						<th class="last_th" width="4%" align="center" valign="middle">
							', $context['can_modify'] ? '<input id="input_grupizin_hepsi" type="checkbox" class="input_check" onclick="invertAll(this, this.form, \'group\');" /><label for="input_grupizin_hepsi"></label>' : '', '
						</th>
					</tr>
				</thead>
				<tbody>';
	...
	foreach ($context['groups'] as $group)
	{
		...
		echo '
						...
						<td align="center">', $group['allow_modify'] && $context['can_modify'] ? '<input id="input_grupizin_' . $group['id'] . '" type="checkbox" name="group[]" value="' . $group['id'] . '" class="input_check" /><label for="input_grupizin_' . $group['id'] . '"></label>' : 'sad', '</td>
					</tr>';
	}
...
function template_edit_profiles()
{
	...
	foreach ($context['profiles'] as $profile)
	{
		...
		echo '
						...
						<td align="center">
							<input id="input_profilsil_', $profile['id'], '" type="checkbox" name="delete_profile[]" value="', $profile['id'], '" ', $profile['can_delete'] ? '' : 'disabled="disabled"', ' class="input_check" /><label for="input_profilsil_', $profile['id'], '"></label>
						</td>
					</tr>';
		$alternate = !$alternate;
	}
					...
					echo '
						<td colspan="3" width="10">
							<div id="group_select_div_', $id_group, '">
								<input type="checkbox" id="group_select_', $id_group, '" name="group_select_', $id_group, '" class="input_check" onclick="determineGroupState(\'', $id_group, '\', this.checked ? \'on\' : \'off\');" style="display: none;" ', $disable_field, '/><label for="group_select_', $id_group, '"></label>
							</div>
						</td>';
				else
					echo '
		...
		foreach ($permissionGroup['permissions'] as $permission)
		{
			...
			else
			{
				echo '
					...
					if (empty($modSettings['permission_enable_deny']) || $context['group']['id'] == -1)
						echo '
						<td valign="top" style="padding-bottom: 2px;"><input type="checkbox" id="select_', $permission['id'], '" name="perm[', $type, '][', $permission['id'], ']"', $permission['select'] == 'on' ? ' checked="checked"' : '', ' onclick="determineGroupState(\'', $id_group, '\');" value="on" class="input_check" ', $disable_field, '/><label for="select_', $permission['id'], '"></label></td>';
					else
						echo '
...
function template_inline_permissions()
{
	...
	foreach ($context['member_groups'] as $group)
	{
		...
		if (empty($modSettings['permission_enable_deny']))
			echo '
					<input id="input_bolum_', $group['id'], '" type="checkbox" name="', $context['current_permission'], '[', $group['id'], ']" value="on"', $group['status'] == 'on' ? ' checked="checked"' : '', ' class="input_check" /><label for="input_bolum_', $group['id'], '"></label>';
		else
			echo '
