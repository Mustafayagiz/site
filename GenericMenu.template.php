	...
	foreach ($menu_context['sections'] as $section)
	{
		...
		foreach ($section['areas'] as $i => $area)
		{
			...
			if($i == $menu_context['current_area'])
			{
				...
				else echo '<i id="menuucgenn" class="fas fa-caret-up fa-2x"></i>';
				echo '<a class="chosen" href="', (isset($area['url']) ? $area['url'] : $menu_context['base_url'] . ';area=' . $i), $menu_context['extra_parameters'], '"><span>', $area['icon'], $area['label'], !empty($area['subsections']) ? '&nbsp;&nbsp;<i class="fas fa-angle-right"></i>' : '', '</span></a>';
			...
			else
			{
				...
				echo '<a href="', (isset($area['url']) ? $area['url'] : $menu_context['base_url'] . ';area=' . $i), $menu_context['extra_parameters'], '"><span>', $area['icon'], $area['label'], !empty($area['subsections']) ? '&nbsp;&nbsp;<i class="fas fa-angle-right"></i>' : '', '</span></a>';
			}
			...
			if(!empty($area['subsections']))
			{
				...
				foreach($area['subsections'] as $sa => $sub)
				{
					...
					echo '<li', (++$additional_items_sub > 6) ? ' class="additional_items"' : '' ,'>';
							if($additional_items_sub != 1)
							{
								echo '<hr>';
							}
							echo '<a ', !empty($sub['selected']) ? 'class="active" ' : '', 'href="', $url, $menu_context['extra_parameters'], '"><span>', $sub['label'], '</span></a>
							</li>';
				}
				echo '</ul>';
			}
