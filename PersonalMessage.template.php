...
<form class="flow_hidden" action="', $scripturl, '?action=pm;sa=pmactions;', $context['display_mode'] == 2 ? 'conversation;' : '', 'f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', '" method="post" accept-charset="', $context['character_set'], '" name="pmFolder">';
	...
	if ($context['get_pmessage']('message', true))
	{
		...
		{
			$window_class = $message['alternate'] == 0 ? 'windowbg' : 'windowbg';
	...
	<div class="', $window_class, ' clear">
		...
		<div class="poster">
			...
			if (!$message['member']['is_guest'])
			{
				...
				echo '<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '" style="color:',$message['member']['group_color'],';">', $message['member']['username'], '</a>';
			...
			if (isset($message['member']['group']) && $message['member']['group'] != '')
				...
				if (!isset($context['disabled_fields']['posts']))
					echo '<li class="postcount">', $txt['member_postcount'], ': ', $message['member']['posts'], '</li><br/>';
		...
		<div class="postarea">
			...
			if($context['can_send_pm'])
			{
				...
				if (!$message['member']['is_guest'])
				{
					...
					<li class="reply_button"><a href="', $scripturl, '?action=pm;sa=send;f=', $context['folder'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';pmsg=', $message['id'], ';u=', $message['member']['id'], '"><i title="Yanıtla" class="fas fa-reply"></i></a></li>


					<li class="quote_button"><a href="', $scripturl, '?action=pm;sa=send;f=', $context['folder'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';pmsg=', $message['id'], ';quote', $context['folder'] == 'sent' ? '' : ';u=' . $message['member']['id'], '"><i title="Alıntıla" class="fas fa-quote-left"></i></a></li>';
				}
			...
			echo '
					<li class="remove_button"><a href="', $scripturl, '?action=pm;sa=pmactions;pm_actions[', $message['id'], ']=delete;f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', addslashes($txt['remove_message']), '?\');"><i title="Sil" class="fas fa-trash-alt"></i></a></li>';

			if (empty($context['display_mode']))
				echo '
					<li class="inline_mod_check"><input type="checkbox" name="pms[]" id="deletedisplay', $message['id'], '" value="', $message['id'], '" onclick="document.getElementById(\'deletelisting', $message['id'], '\').checked = this.checked;" class="input_check" /><label for="deletedisplay', $message['id'], '"></label></li>';
	...
	<thead>
		<tr class="catbg">
			...
			<th align="center" width="4%" class="last_th">
				<input id="input_check" type="checkbox" onclick="invertAll(this, this.form);" class="input_check" /><label for="input_check"></label>
			</th>
		</tr>
	</thead>
	<tbody>';
		...
		<fieldset id="advanced_search">
			...
			<div class="roundframe">
				...
				<dl id="search_options">
					...
					<dd class="options">
						<input type="checkbox" name="show_complete" id="show_complete" value="1"', !empty($context['search_params']['show_complete']) ? ' checked="checked"' : '', ' class="input_check" /><label for="show_complete">', $txt['pm_search_show_complete'], '</label><br />
						<input type="checkbox" name="subject_only" id="subject_only" value="1"', !empty($context['search_params']['subject_only']) ? ' checked="checked"' : '', ' class="input_check" /><label for="subject_only">', $txt['pm_search_subject_only'], '</label>
					</dd>
...
function template_send()
{
	...
	echo '
				<p><input type="checkbox" name="outbox" id="outbox" value="1" tabindex="', $context['tabindex']++, '"', $context['copy_to_outbox'] ? ' checked="checked"' : '', ' class="input_check" /><label for="outbox"> ', $txt['pm_save_outbox'], '</label></p>
	...
	echo '
		<script type="text/javascript" src="', $settings['theme_url'], '/scripts/PersonalMessage.js?fin20"></script>
...
function template_labels()
{
	...
	if (count($context['labels']) > 2)
		echo '
					<input id="input_check" type="checkbox" class="input_check" onclick="invertAll(this, this.form);" /><label for="input_check"></label>';
