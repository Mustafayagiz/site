...
function template_main()
{
	...
	{
		echo '<div id="cevrimiciliste">Konudaki kullanıcılar</div>
		<div id="cevrimiciuyeler">Bu konuyu gezen kullanıcılar:&nbsp;';
		// Show just numbers...?
		...
		{
			echo $txt['who_and'], $context['view_num_guests'], ' ', $context['view_num_guests'] == 1 ? 'ziyaretçi' : 'ziyaretçiler', '';
		}
		echo '</div>';
	}
	...
	echo '
			<div class="pagesection">', template_button_strip($normal_buttons, 'left'), '
	...
	echo '<form action="', $scripturl, '?action=quickmod2;topic=', $context['current_topic'], '.', $context['start'], '" method="post" accept-charset="', $context['character_set'], '" name="quickModForm" id="quickModForm" style="margin: 0;" onsubmit="return oQuickModify.bInEditMode ? oQuickModify.modifySave(\'' . $context['session_id'] . '\', \'' . $context['session_var'] . '\') : false">';
	...
	while ($message = $context['get_message']())
	{
		...
		if($message['id'] == $context['first_message'])
		{
			echo '<div class="post_wrapper"><div class="poster"><ul class="reset smalltext" id="msg_', $message['id'], '_extra_info">';
		}
		else
		{
			echo '<div class="post_wrapperr"><div class="poster"><ul class="reset smalltext" id="msg_', $message['id'], '_extra_info">';
		}
		...
		if(!empty($settings['show_user_images']) && empty($options['show_no_avatars']))
		{
			...
			if(empty($message['member']['avatar']['image']))
			{
				echo '<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
									<img src="', $settings['images_url'], '/avatar.png"/>
								</a>';
			}
			else
				echo '<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
		...
		if(!empty($modSettings['onlineEnable']) && !$message['member']['is_guest'])
		{
			if($message['member']['online']['is_online'])
				echo '<div style="background-color: #55BF00;" class="cevrimicidurum"></div>';
			else
				echo '<div style="background-color: #C1C1C1;" class="cevrimicidurum"></div>';
		}
		// Show the member's custom title, if they have one.
		if (!empty($message['member']['title']))
			echo '<li class="title">', $message['member']['title'], '</li>';
		...
		if(!$message['member']['is_guest'])
		{
			...
			if (!isset($context['disabled_fields']['posts']))
				echo '
								<li style="color:#383838;" class="postcount">', $txt['member_postcount'], ': <a style="color:#666666; font-size: normal;font-weight: normal;" href="', $scripturl, '?action=profile;area=showposts;u=', $message['member']['id'], '">', $message['member']['posts'], '</a></li>';
			...
			if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '' && !isset($context['disabled_fields']['gender']))
				echo '<li class="gender">', $txt['gender'], ': ', $message['member']['gender']['image'], '</li>';
			...
			if ($settings['show_profile_buttons'])
			{
				...
				echo '<ul style="color:#383838;">İletişim: ';

				if($context['can_send_pm'])
				{
					echo '<li><a href="', $scripturl, '?action=pm;sa=send;u=', $message['member']['id'], '" title="', $message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline'], '"><i style="font-size: 16px; font-weight: 400; color:#666666;" class="far fa-comment-dots"></i></a></li>';
				}
			...
			if($context['can_moderate_forum'] && !empty($message['member']['ip']))
				echo '<li><a style="font-size: 10px;" href="', $scripturl, '?action=', !empty($message['member']['is_guest']) ? 'trackip' : 'profile;area=tracking;sa=ip;u=' . $message['member']['id'], ';searchip=', $message['member']['ip'], '">', $message['member']['ip'], '</a></li>';
			// Or, should we show it because this is you?
			elseif ($message['can_see_ip'])
				echo '<li><a style="font-size: 10px;" href="', $scripturl, '?action=helpadmin;help=see_member_ip" onclick="return reqWin(this.href);" class="help">', $message['member']['ip'], '</a></li>';
			echo '</ul><br/>';
						...
						<div class="postarea">
							<div class="flow_hidden">
								<div class="keyinfo">
									...
									<div style="font-size: 0.80em;" class="smalltext"><i style="color: #C8C8C8;" class="fas fa-file fa-xs"></i><strong>', $message['member']['link'], '</strong> » ', $message['time'], '</div>
		...
		if ($context['can_report_moderator'])
			echo '<li><a href="', $scripturl, '?action=reporttm;topic=', $context['current_topic'], '.', $message['counter'], ';msg=', $message['id'], '"><i title="Moderatöre rapor et" style="color: #9c2525;" class="fas fa-exclamation-circle"></i></a></li>';

		// Can they reply? Have they turned on quick reply?
		if ($context['can_quote'] && !empty($options['display_quick_reply']))
			echo '
									<li class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" onclick="return oQuickReply.quote(', $message['id'], ');"><i title="Alıntıla" class="fas fa-quote-left"></i></a></li>';

		// So... quick reply is off, but they *can* reply?
		elseif ($context['can_quote'])
			echo '
									<li class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '"><i title="Alıntıla" class="fas fa-quote-left"></i></a></li>';

		// Can the user modify the contents of this post?
		if ($message['can_modify'])
			echo '
									<li class="modify_button"><a style="color: #323A45;" href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], '"><i title="İletiyi düzenle" class="fas fa-edit"></i></a></li>';

		// How about... even... remove it entirely?!
		if ($message['can_remove'])
			echo '
									<li class="remove_button"><a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');"><i title="Sil" class="fas fa-trash-alt"></i></a></li>';

		// What about splitting it off the rest of the topic?
		if ($context['can_split'] && !empty($context['real_num_replies']))
			echo '
									<li class="split_button"><a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '"><i title="Konuyu böl" class="fas fa-arrows-alt-v"></i></a></li>';
		...
		if (!empty($options['display_quick_mod']) && $options['display_quick_mod'] == 1 && $message['can_remove'])
			echo '<li class="inline_mod_check" style="display: none;" id="in_topic_mod_check_', $message['id'], '"></li>';
	...
	echo '<div style="width: 100%;"><div class="plainbox" id="display_jump_to">&nbsp;</div>';
	echo '<div id="moderationbuttons">', template_button_strip($mod_buttons, 'bottom', array('id' => 'moderationbuttons_strip')), '</div>';
			...
			<div class="tborder" id="quickreplybox">
				<div class="cat_bar">
					<h3 class="catbg">
						...
						<a href="javascript:oQuickReply.swap();">', $txt['quick_reply'], '</a></span>
					</h3>
				</div>
