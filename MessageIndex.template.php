...
function template_main()
{
	...
	{
		echo '<div id="cevrimiciliste">Kategorideki kullanıcılar</div>
		<div id="cevrimiciuyeler">Bu kategoriyi gezen kullanıcılar:&nbsp;';
	...
	if (!empty($context['boards']) && (!empty($options['show_children']) || $context['start'] == 0))
	{
		echo '
		<br/>
	<div class="tborder topic_table" id="messageindexx">
			<table cellspacing="0" class="table_grid">
			<thead>
			<tr class="catbg">
				<th style="text-align: center;" scope="col" width="5%" class="first_th">&nbsp;</th>
				<th style="padding-left: 15px; font-size: 0.85em;" scope="col" width="45%" class="lefttext"><strong>', $txt['parent_boards'], '</th>
				<th style="text-align: center; font-size: 0.85em;" scope="col" class="lefttext" width="5%">Konular</th>
				<th style="text-align: center; font-size: 0.85em;" scope="col" class="lefttext" width="5%">İletiler</th>
				<th style="font-size: 0.85em;" scope="col" class="lefttext" width="18%">Son İleti</th>
			</tr></thead>
		...
		foreach ($context['boards'] as $board)
		{
			...
			echo '
					...
					<td class="subject windowbg2">
						<strong><a class="subject" href="', $board['href'], '" name="b', $board['id'], '">', $board['name'], '</a></strong>';
			...
			echo '<td class="stats windowbg"><p>', comma_format($board['posts']), '</p></td>';
				echo '<td class="lastpost windowbg2">';
	...
	<div class="tborder topic_table" id="messageindex">
		...
		{
			echo '<th style="text-align: center;" scope="col" width="5%" class="first_th"></th>';
			echo '<th style="padding-left: 8px;" scope="col" width="45%" class="lefttext">', $txt['subject'], '</th>';
			if(empty($context['can_quick_mod']))
				echo '<th style="text-align: center;" scope="col" class="lefttext" width="5%" colspan="1">Yanıtlar</th>';
			else
				echo '<th style="text-align: center;" scope="col" class="lefttext" width="5%" colspan="1">Yanıtlar</th>';

			echo '<th style="font-size: 0.85em; text-align: center;" scope="col" width="5%">Görüntülemeler</th>';
			// Show a "select all" box for quick moderation?
			if(empty($context['can_quick_mod']))
				echo '<th scope="col" class="lefttext last_th" width="18%">Son İleti</th>';
			else
				echo '<th scope="col" class="lefttext" width="18%"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></th>';

			// Show a "select all" box for quick moderation?
			if (!empty($context['can_quick_mod']) && $options['display_quick_mod'] == 1)
				echo '<th align="center" scope="col" class="last_th" width="5%"><input id="input_check" type="checkbox" onclick="invertAll(this, this.form, \'topics[]\');" class="input_check" /><label for="input_check"></label></th>';

			// If it's on in "image" mode, don't show anything but the column.
			elseif (!empty($context['can_quick_mod']))
				echo '<th class="last_th" width="4%">&nbsp;</th>';
		}
		...
		foreach ($context['topics'] as $topic)
		{
				...
				<tr>
					...
					<td class="subject ', $alternate_class, '">
						<div ', (!empty($topic['quick_mod']['modify']) ? 'id="topic_' . $topic['first_post']['id'] . '" onmouseout="mouse_on_div = 0;" onmouseover="mouse_on_div = 1;" ondblclick="modify_topic(\'' . $topic['id'] . '\', \'' . $topic['first_post']['id'] . '\');"' : ''), '>
							', $topic['is_sticky'] ? '' : '', '<span style="font-size: 1.0em; font-weight: 600;" id="msg_' . $topic['first_post']['id'] . '">', $topic['first_post']['link'], (!$context['can_approve_posts'] && !$topic['approved'] ? '&nbsp;<em>(' . $txt['awaiting_approval'] . ')</em>' : ''), '</span>', $topic['is_sticky'] ? '' : '';
			...
			echo '<p>', $txt['started_by'], ' ', $topic['first_post']['member']['link'], ' »
								<small id="pages' . $topic['first_post']['id'] . '">', $topic['pages'], ' ', $topic['first_post']['time'],' </small>
							</p>
						</div>
					</td>
					<td class="icon2 ', $color_class, '">', $topic['replies'], '</td>
					<td class="stats ', $color_class, '">', $topic['views'], '</td>
					<td class="lastpost ', $alternate_class, '">
						', $txt['by'], ' ', $topic['last_post']['member']['link'], ' <a href="', $topic['last_post']['href'], '"><i style="font-size: 0.7em; font-weight: bold;" class="fas fa-angle-right"></i></a><br />
			...
			if (!empty($context['can_quick_mod']))
			{
				echo '<td class="moderation ', $color_class, '" align="center">';
				if ($options['display_quick_mod'] == 1)
					echo '<input id="input_check_', $topic['id'], '" type="checkbox" name="topics[]" value="', $topic['id'], '" class="input_check" /><label for="input_check_', $topic['id'], '"></label>';
				else
				{
		...
		if (!empty($context['can_quick_mod']) && $options['display_quick_mod'] == 1 && !empty($context['topics']))
		{
			...
			echo '
						<input style="margin-top: 25px;" type="submit" value="Git" onclick="return document.forms.quickModForm.qaction.value != \'\' &amp;&amp; confirm(\'', $txt['quickmod_confirm'], '\');" class="button_submit qaction" />
					</td>
				</tr>';
		}
	...
	<div class="pagesection">
		', template_button_strip($normal_buttons, 'left'), '
		<div class="pagelinks">', $context['page_index'], !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . '&nbsp;&nbsp;<a href="#top"><strong>' . $txt['go_up'] . '</strong></a>' : '', '</div>
	</div>';
	}
