...
function template_maintain_database()
{
	...
	if ($db_type == 'sqlite')
		...
		echo '
					<p><input type="checkbox" name="struct" id="struct" onclick="document.getElementById(\'submitDump\').disabled = !document.getElementById(\'struct\').checked &amp;&amp; !document.getElementById(\'data\').checked;" class="input_check" checked="checked" /><label for="struct">', $txt['maintain_backup_struct'], '</label><br />
					<input type="checkbox" name="data" id="data" onclick="document.getElementById(\'submitDump\').disabled = !document.getElementById(\'struct\').checked &amp;&amp; !document.getElementById(\'data\').checked;" checked="checked" class="input_check" /><label for="data">', $txt['maintain_backup_data'], '</label><br />
					<input type="checkbox" name="compress" id="compress" value="gzip" checked="checked" class="input_check" /><label for="compress">', $txt['maintain_backup_gz'], '</label></p>
...
function template_maintain_members()
{
	...
	foreach ($context['membergroups'] as $group)
		echo '
						<input type="checkbox" name="groups[', $group['id'], ']" id="groups', $group['id'], '" checked="checked" class="input_check" /><label for="groups', $group['id'], '">', $group['name'], '</label><br />';
...
function template_maintain_topics()
{
	...
	if (!empty($modSettings['enableStickyTopics']))
		echo '
					<p>
						<input type="checkbox" name="delete_old_not_sticky" id="delete_old_not_sticky" class="input_check" checked="checked" /><label for="delete_old_not_sticky">', $txt['maintain_old_are_not_stickied'], '</label><br />
					</p>';
	...
	foreach ($context['categories'] as $category)
	{
		...
		foreach ($category['boards'] as $board)
			echo '
									<li style="margin-', $context['right_to_left'] ? 'right' : 'left', ': ', $board['child_level'] * 1.5, 'em;"><input type="checkbox" name="boards[', $board['id'], ']" id="boards_', $board['id'], '" checked="checked" class="input_check" /><label for="boards_', $board['id'], '">', $board['name'], '</label></li>';
