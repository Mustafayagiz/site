...
function template_main()
{
	...
	<div class="main_section" id="memberlist">
		...
		<div class="pagesection">
			', template_button_strip($memberlist_buttons, 'left'), '
	...
	foreach ($context['columns'] as $column)
	{
		...
		elseif ($column['selected'])
			echo '
					<th scope="col" class="', isset($column['class']) ? ' ' . $column['class'] : '', '" style="width: 25%;"' . (isset($column['colspan']) ? ' colspan="' . $column['colspan'] . '"' : '') . ' nowrap="nowrap">
	...
	if (!empty($context['members']))
	{
		foreach ($context['members'] as $member)
		{
			...
			echo '<tr ', empty($member['sort_letter']) ? '' : ' id="letter' . $member['sort_letter'] . '"', '>
					<td class="windowbg lefttext">', $member['link'], '';
			if($member['online']['is_online'])
				echo '<div style="background-color: #55BF00;" class="cevrimicidurum"></div></td>';
			else
				echo '<div style="background-color: #C1C1C1;" class="cevrimicidurum"></div></td>';
		// Group and date.
		echo '
...
function template_search()
{
	...
	{
		echo '
							<input type="checkbox" name="fields[]" id="fields-', $id, '" value="', $id, '" ', in_array($id, $context['search_defaults']) ? 'checked="checked"' : '', ' class="input_check" /><label for="fields-', $id, '">', $title, '</label><br />';
	// Half way through?
